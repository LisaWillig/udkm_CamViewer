...
import numpy as np
try:
    from modules.ThorlabsCamera.uc480.uc480_h import *
except ImportError:
    from ThorlabsCamera.uc480.uc480_h import *
        ...
        if library is None:
            if (platform.architecture()[0] == "32bit"):
                ...
                if _linux:
                    ...
                    try:
                        self._lib = ctypes.cdll.LoadLibrary("modules\\ThorlabsCamera\\uc480\\uc480_64.dll")
                    except OSError:
                        try:
                            self._lib = ctypes.cdll.LoadLibrary("ThorlabsCamera\\uc480\\uc480_64.dll")
                        except OSError:
                            try:
                                self._lib = ctypes.cdll.LoadLibrary("uc480\\uc480_64.dll")
                            except OSError:
